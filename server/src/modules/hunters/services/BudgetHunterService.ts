...
class BudgetHunter extends Subject {
  ...
  constructor({ user, password, license, url }: BudgetHunterParams) {
    ...
    this.huntersRepository = {} as IHuntersRepository;
  }
  ...
  public async getBudgets(params: HuntParams): Promise<BudgetHuntedDTO[]> {
    try {
  ...
  public async toHunt(
    { filter, huntPagesQuantity }: HuntParams,
    hunterId: string,
    hunterRepository: IHuntersRepository,
  ): Promise<BudgetHuntedDTO[]> {
    try {
      this.huntersRepository = hunterRepository;
      await this.huntersRepository.save({
        id: hunterId,
        statusProgressHunting: StatusProgressDTO.IN_PROGRESS,
      });

      await this.configure();
      const budgetHunted = await this.getBudgets({ filter, huntPagesQuantity });

      await this.huntersRepository.save({
        id: hunterId,
        budgetsHunted: budgetHunted,
        statusProgressHunting: StatusProgressDTO.COMPLETED,
        statusProgressNormalized: StatusProgressDTO.IN_PROGRESS,
      });
      ...
      await this.huntersRepository.save({
        id: hunterId,
        statusProgressHunting: StatusProgressDTO.ERROR,
        errors: [error],
      });
